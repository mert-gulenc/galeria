...
function Image({ __web, index = 0, children, style, dynamicAspectRatio = false, }) {
    ...
    const getNodeAspectRatio = (node) => {
        ...
        if (imageNode) {
            return (imageNode.getBoundingClientRect().width /
      ...
      <PopupModal visible={isOpen} onClose={() => setIsOpen(false)}>
        <WindowDimensions>
          {(dimensions) => {
            ...
            const width = imageRatio > windowRatio
                ? dimensions.width
                : dimensions.height * imageRatio;
            const height = imageRatio > windowRatio
            ...
            return (<motion.div style={{
                    ...
                    alignItems: 'center',
                }} initial={{ backgroundColor: background + '00' }} animate={{ backgroundColor: background }} exit={{ backgroundColor: background + '00' }}>
        ...
        <motion.div style={{
          ...
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M19 6.41L17.59 5L12 10.59L6.41 5L5 6.41L10.59 12L5 17.59L6.41 19L12 13.41L17.59 19L19 17.59L13.41 12L19 6.41Z" fill={foreground}/>
          </svg>
        </motion.div>

        <OnScrollOnce onScroll={() => {
            isOpen && setIsOpen(false);
        }}/>
      </PopupModal>
    </>);
}
function Root({ children, urls, theme = 'dark', ids, }) {
